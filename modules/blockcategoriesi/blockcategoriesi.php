...
class BlockCategoriesi extends Module
{
	function __construct()
	{
		...
		$this->description = $this->l('Adds a block featuring product categories - catalogo-onlienrsi.net');
	}
   ...
   public function postProcess()
    {
        $errors = '';
        if (Tools::isSubmit('submitCategoriesi')) {
            if ($maxDepth = Tools::getValue('maxDepth')) {
                Configuration::updateValue(
                    'BLOCK_CATEGI_MAX_DEPTH',
                    $maxDepth
                );
            } elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP) {
                Configuration::deleteFromContext('BLOCK_CATEGI_MAX_DEPTH');
            }

            if ($dhtml = Tools::getValue('dhtml')) {
                Configuration::updateValue(
                    'BLOCK_CATEGI_DHTML',
                    $dhtml
                );
            } elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP) {
                Configuration::deleteFromContext('BLOCK_CATEGI_IMAGE');
            }
            if ($image = Tools::getValue('image')) {
                ...
                Configuration::deleteFromContext('COOLSHARE_HEIGHT');
            }
    ...
    public function renderForm()
    {
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'image',
                        'desc' => $this->l('Height of the module (60 default, in px)'),
                    ),
                    array(
                        'type' => 'switch',
        ...
        $helper->submit_action = 'submitCategoriesi';
    ...
    {
        return array(
            'maxDepth' => Tools::getValue(
                'maxDepth',
                Configuration::get('BLOCK_CATEGI_MAX_DEPTH')
            ),
            'image' => Tools::getValue(
                'image',
                Configuration::get('BLOCK_CATEGI_IMAGE')
            ),
            'dhtml' => Tools::getValue(
                'dhtml',
                Configuration::get('BLOCK_CATEGI_DHTML')
            ),
        );
    }
...
<img alt="" border="0" src="https://www.paypalobjects.com/WEBSCR-640-20110401-1/en_US/i/scr/pixel.gif" width="1" height="1">
	...
	function hookLeftColumn($params)
	{
		...
		if (_PS_VERSION_ < "1.7.0.0")
			...
			$smarty->assign('branche_tpl_path', _PS_MODULE_DIR_.'blockcategoriesi/category-tree-branch7.tpl');
		...
		return $this->display(__FILE__, 'blockcategoriesi.tpl');
			...
			return $this->display(__FILE__, 'blockcategoriesi7.tpl');
		}
		}
...
$id_customer = (int)($params['cookie']->id_customer);
		...
		if (_PS_VERSION_ < "1.7.0.0") {
			...
			$display = $this->display(__FILE__, 'blockcategoriesi7.tpl', $smartyCacheId);
		}
		...
		if (!$this->isCached('blockcategoriesi.tpl', $smartyCacheId))
		{
			...
			$this->smarty->assign('branche_tpl_path', _PS_THEME_DIR_.'modules/blockcategoriesi/category-tree-branch.tpl');
				...
				$this->smarty->assign('branche_tpl_path', _PS_MODULE_DIR_.'blockcategoriesi/category-tree-branch7.tpl');
