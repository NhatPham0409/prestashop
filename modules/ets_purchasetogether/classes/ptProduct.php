                    ...
                    if (!empty($combinations)) 
                    {
                        foreach ($combinations as $k => $combination) 
                        {
                            ...
                            if (empty($results[$combination['id_product_attribute']]['image'])) {
                                $results[$combination['id_product_attribute']]['image'] = !empty($combination['id_image'])?str_replace('http://', Tools::getShopProtocol(), $context->link->getImageLink($item['link_rewrite'], $combination['id_image'], ImageType::getFormattedName('home'))): str_replace('http://', Tools::getShopProtocol(), $context->link->getImageLink($item['link_rewrite'], $item['id_image']));
                            }
                        }
                    } 
                    else 
