...
*}
{if isset($isQuickview) && !empty($isQuickview)}
    <button class="wishlist-button-add wishlist-button-product quickview-button"
            onclick="WishlistCart('wishlist_block_list', 'add', '{$product.id_product|intval}', '{$product.id_product_attribute|intval}', 1,'{if isset($wishlists) && is_array($wishlists) && count($wishlists) > 1}{$wishlist.id_wishlist}{/if}');
                    return false;">
        ...
        <div class="wishlist">
            {foreach name=wl from=$wishlists item=wishlist}
                {if $smarty.foreach.wl.first}
                    ...
                    <tbody>
                {/if}
                <tr title="{$wishlist.name}" value="{$wishlist.id_wishlist}" onclick="WishlistCart('wishlist_block_list', 'add', '{$product.id_product|intval}', '{$product.id_product_attribute|intval}', 1, '{$wishlist.id_wishlist}');">
                    <td>
